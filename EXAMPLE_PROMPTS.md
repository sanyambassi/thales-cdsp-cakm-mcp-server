...
**3. Can you enable TDE on oracle_cdb2? If so, automatically restart the database, if needed. Use the following CipherTrust manager credentials to setup TDE:**
```
Can you enable TDE on oracle_cdb2? If so, automatically restart the database, if needed. Use the following CipherTrust manager credentials to setup TDE:
User- tdeuser
...
Skip auto-login wallet setup/creation. Do you need any other information from me before proceeding?
```
...
**6. Do you have the ability to perform migration or reverse migration? Are any Oracle databases ready for either migration?**
```
Do you have the ability to perform migration or reverse migration? Are any Oracle databases ready for either migration?
```
...
```
Generate a key rotation schedule for prod_sql with 90-day intervals. Include both master keys and database encryption keys. Format as cron jobs.
```
...
```
Try to encrypt a non-existent database called "InvalidDB" on prod_sql.
```

```
Check encryption status for a database that doesn't exist.
```
