"""
Oracle TDE configuration tools with enhanced parameter management
"""
...
def register_oracle_configuration_tools(server: FastMCP, db_manager):
    ...
    @server.tool()
    async def get_oracle_tde_configuration(
        oracle_connection: str,
        include_spfile: bool = True
    ) -> str:
        """
        Get current Oracle TDE configuration parameters with enhanced details.
        
        Args:
            oracle_connection: Oracle database connection name
            include_spfile: Include SPFILE values that may differ from memory
            
        Returns:
            JSON string containing TDE configuration parameters.
        """
        try:
            logger.info(f"=== get_oracle_tde_configuration called ===")
            ...
            logger.info(f"=== get_oracle_tde_configuration completed ===")
            return json.dumps(result_data, indent=2)
            
        except Exception as e:
            logger.error(f"Error getting TDE configuration: {e}", exc_info=True)
            return json.dumps({
                "success": False,
                "error": str(e),
    ...
    @server.tool()
    async def set_oracle_tde_parameters(
        oracle_connection: str,
        ...
        """
        Set Oracle TDE parameters with validation and restart handling.
        
        Args:
            oracle_connection: Oracle database connection name
            wallet_root: Wallet root directory path
            tde_configuration: "HSM" | "FILE" | "HSM|FILE" | "FILE|HSM"
            encrypt_new_tablespaces: "DDL" | "ALWAYS" | "CLOUD_ONLY" | "MANUAL"
            force_restart: Automatically restart database if required
            validate_only: Only validate parameters without setting them
            
        Returns:
            JSON string containing operation results.
        """
        try:
            logger.info(f"=== set_oracle_tde_parameters called ===")
            ...
            if tde_configuration:
                ...
                if current_value != tde_configuration:
                    tde_sql = f"ALTER SYSTEM SET TDE_CONFIGURATION='{tde_configuration}' SCOPE=BOTH"
            ...
            logger.info(f"=== set_oracle_tde_parameters completed ===")
            return json.dumps(result_data, indent=2)
            
        except Exception as e:
            logger.error(f"Error setting TDE parameters: {e}", exc_info=True)
            return json.dumps({
                "success": False,
                "error": str(e),
                "error_type": type(e).__name__
    ...
    @server.tool()
    async def verify_oracle_tde_setup(
        oracle_connection: str
    ) -> str:
        """
        Verify Oracle TDE setup and provide recommendations.
        
        Args:
            oracle_connection: Oracle database connection name
            
        Returns:
            JSON string containing verification results and recommendations.
        """
        try:
            logger.info(f"=== verify_oracle_tde_setup called ===")
            ...
            logger.info(f"=== verify_oracle_tde_setup completed ===")
            return json.dumps(result_data, indent=2)
            
        except Exception as e:
            logger.error(f"Error verifying TDE setup: {e}", exc_info=True)
            return json.dumps({
                "success": False,
                "error": str(e),
                "error_type": type(e).__name__
