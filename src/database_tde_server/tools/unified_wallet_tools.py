"""
Unified Oracle wallet management tools
"""
...
def register_unified_wallet_tools(server: FastMCP, db_manager):
    """Register unified Oracle wallet management tools with the MCP server"""
    ...
    async def manage_oracle_wallet(
        ...
        Args:
            ...
            operation: Wallet operation - "open" | "close" | "status"
        ...
        try:
            ...
            # Helper function to check if autologin is configured
            def has_autologin_wallet(wallets):
                return any(w.get("WALLET_TYPE") == "AUTOLOGIN" for w in wallets)
            
            # Helper function to check if autologin is configured (even when closed)
            def has_autologin_configuration(wallets):
                # Check if any wallet has AUTOLOGIN type (when open)
                if has_autologin_wallet(wallets):
                    return True
            ...
            # Helper function to check if wallet is already open for the container
            def is_wallet_open_for_container(wallets):
                return any(is_wallet_open(w) for w in wallets)
            
            if operation == "status":
                # Operation 1: Check wallet status and return response
                result_data = {
                    "success": True,
                    ...
                    "summary": {
                        ...
                        "wallet_open_for_container": is_wallet_open_for_container(file_wallets + hsm_wallets)
                    },
                    "timestamp": datetime.now().isoformat()
                }
                
            elif operation == "close":
                # Operation 2: Close wallet with proper logic
                # Check if any wallet is open for this container
                if not is_wallet_open_for_container(file_wallets + hsm_wallets):
                    # 2.2: Wallet already closed
                    result_data = {
                        "success": True,
                    ...
                    # 2.3: Close wallet with single query
                    close_results = []
                    ...
                    if has_autologin:
                        # 2.3.1: Close with autologin (no credentials needed)
                        if container == "ALL":
                ...
                if is_wallet_open_for_container(file_wallets + hsm_wallets):
                    ...
                    # 3.3: Open wallet with single query
                    # Check if autologin is configured
                    has_autologin = has_autologin_configuration(file_wallets + hsm_wallets)
                ...
                return json.dumps({
                    "success": False,
                    "error": f"Invalid operation: {operation}. Must be 'open', 'close', or 'status'"
                })
    ...
    @server.tool()
    async def backup_oracle_wallet(
        oracle_connection: str,
        ...
        """
        Backup Oracle wallet files using Oracle's built-in backup capabilities.
        
        Args:
            oracle_connection: Oracle database connection name
            wallet_location: Source wallet location (auto-detected if not provided)
            backup_location: Backup destination (defaults to same directory with .backup suffix)
            include_autologin: Include auto-login wallet files (default: True)
        ...
        """
        try:
            logger.info(f"=== backup_oracle_wallet called ===")
            ...
            if backup_result.get("success", False):
                ...
                result_data = {
                    ...
                    "manual_backup_instructions": {
                        "note": "For file-level backup, use these commands on the Oracle server:",
                        "commands": [
                            f"# Connect to Oracle server and run:",
                            f"cd {wallet_location}",
                            f"# Backup main wallet files:",
                            f"cp tde/ewallet.p12 ewallet.p12.backup_{backup_suffix}",
                            f"cp tde/cwallet.sso cwallet.sso.backup_{backup_suffix}",
                            f"# Backup any additional wallet files:",
                            f"cp tde/*.p12 *.p12.backup_{backup_suffix} 2>/dev/null || true",
                            f"# Verify backup:",
                            f"ls -la *.backup_{backup_suffix}"
                        ]
                    },
                    "timestamp": datetime.now().isoformat()
                }
                ...
                result_data = {
                    ...
                    "manual_backup_instructions": {
                        "note": "Oracle backup failed. Use manual backup instead:",
                        "commands": [
                            f"# Connect to Oracle server and run:",
                            f"cd {wallet_location or '$ORACLE_BASE/admin/$ORACLE_SID/wallet'}",
                            f"# Backup main wallet files:",
                            f"cp tde/ewallet.p12 ewallet.p12.backup_{backup_suffix or datetime.now().strftime('%Y%m%d_%H%M%S')}",
                            f"cp tde/cwallet.sso cwallet.sso.backup_{backup_suffix or datetime.now().strftime('%Y%m%d_%H%M%S')}",
                            f"# Backup any additional wallet files:",
                            f"cp tde/*.p12 *.p12.backup_{backup_suffix or datetime.now().strftime('%Y%m%d_%H%M%S')} 2>/dev/null || true",
                            f"# Verify backup:",
                            f"ls -la *.backup_{backup_suffix or datetime.now().strftime('%Y%m%d_%H%M%S')}"
                        ]
                    },
                    "timestamp": datetime.now().isoformat()
                }
            
            logger.info(f"=== backup_oracle_wallet completed ===")
            return json.dumps(result_data, indent=2)
            
        except Exception as e:
            logger.error(f"Error backing up wallet: {e}", exc_info=True)
            return json.dumps({
                "success": False,
                "error": str(e),
                "error_type": type(e).__name__,
                "manual_backup_instructions": {
                    "note": "Backup failed. Use manual backup instead:",
                    "commands": [
                        f"# Connect to Oracle server and run:",
                        f"cd {wallet_location or '$ORACLE_BASE/admin/$ORACLE_SID/wallet'}",
                        f"# Backup main wallet files:",
    ...
    @server.tool()
    async def manage_oracle_autologin(
        oracle_connection: str,
        operation: str,
        ...
        """
        Comprehensive Oracle auto-login wallet management.
        
        Args:
            oracle_connection: Oracle database connection name
            operation: Operation type:
                - "create" - Create auto-login wallet from existing keystore
                - "setup" - Complete auto-login setup from scratch (creates software wallet first)
                - "setup_hsm" - Setup auto-login for HSM migration (from existing software wallet)
                - "update" - Update auto-login wallet password
                - "update_secret" - Update HSM credentials in auto-login wallet
                - "remove" - Remove auto-login wallet
            ...
            software_keystore_password: Password for software keystore (required for setup operations)
            new_ciphertrust_username: New CipherTrust username for update operation
            new_ciphertrust_password: New CipherTrust password for update operation
            new_ciphertrust_domain: New CipherTrust domain for update operation
            auto_restart: If True, attempts database restart via SSH (for setup operations)
            
        Returns:
            JSON string containing auto-login operation results.
        """
        try:
            logger.info(f"=== manage_oracle_autologin called ===")
            ...
            if operation == "create":
                ...
                logger.info("Step 1: Deleting old secret...")
                delete_secret_sql = f"""
                ADMINISTER KEY MANAGEMENT DELETE SECRET FOR CLIENT 'HSM_PASSWORD' 
                IDENTIFIED BY "{software_keystore_password}" 
                WITH BACKUP
                """
                delete_result = await db_handler.execute_sql(delete_secret_sql, "CDB$ROOT")
                steps.append({
                    "step": "delete_old_secret",
                ...
                if not delete_result.get("success", False):
                    return json.dumps({
                        "success": False,
                        "error": f"Failed to delete old secret: {delete_result.get('error', 'Unknown error')}",
                        "steps": steps
                    })
                
                # Step 2: Add new secret
                logger.info("Step 2: Adding new secret...")
                add_secret_sql = f"""
                ADMINISTER KEY MANAGEMENT ADD SECRET '{wallet_password}' 
                FOR CLIENT 'HSM_PASSWORD' 
                IDENTIFIED BY "{software_keystore_password}" 
                WITH BACKUP
                """
                add_result = await db_handler.execute_sql(add_secret_sql, "CDB$ROOT")
                steps.append({
                    "step": "add_new_secret",
                ...
                if not add_result.get("success", False):
                    return json.dumps({
                        "success": False,
                        "error": f"Failed to add new secret: {add_result.get('error', 'Unknown error')}",
                        "steps": steps
                    })
                
                # Step 3: Recreate auto-login wallet
                logger.info("Step 3: Recreating auto-login wallet...")
                create_autologin_sql = f"""
                ADMINISTER KEY MANAGEMENT CREATE AUTO_LOGIN KEYSTORE FROM KEYSTORE
                IDENTIFIED BY "{software_keystore_password}"
                """
                autologin_result = await db_handler.execute_sql(create_autologin_sql, "CDB$ROOT")
                steps.append({
                    "step": "recreate_autologin_wallet",
                    "success": autologin_result.get("success", False)
                })
                ...
                if not autologin_result.get("success", False):
                    return json.dumps({
                        "success": False,
                        "error": f"Failed to recreate auto-login wallet: {autologin_result.get('error', 'Unknown error')}",
                        "steps": steps
                    })
                ...
                result_data = {
                    ...
                    "wallet_location": wallet_location,
                    "steps": steps,
                    "note": "HSM secret updated in auto-login wallet successfully.",
                    "timestamp": datetime.now().isoformat()
                }
                ...
                return json.dumps({
                    "success": False,
                    "error": f"Invalid operation: {operation}. Must be 'create', 'update', 'remove', 'setup', 'setup_hsm', or 'update_secret'"
                })
            
            logger.info(f"=== manage_oracle_autologin completed ===")
            ...
            logger.error(f"Error managing auto-login: {e}", exc_info=True)
            return json.dumps({
                "success": False,
                "error": str(e),
                "error_type": type(e).__name__
    ...
    @server.tool()
    async def merge_oracle_wallets(
        oracle_connection: str,
        source_wallet: str,
        target_wallet: str,
        ciphertrust_username: str,
        ...
        """
        Merge Oracle wallets by copying keys from source to target.
        
        Args:
            oracle_connection: Oracle database connection name
            source_wallet: Source wallet location
            target_wallet: Target wallet location
            ciphertrust_username: CipherTrust Manager username
            ciphertrust_password: CipherTrust Manager password
            ciphertrust_domain: CipherTrust Manager domain (default: root)
            
        Returns:
            JSON string containing merge operation results.
        """
        try:
            logger.info(f"=== merge_oracle_wallets called ===")
            logger.info(f"Source: {source_wallet}, Target: {target_wallet}")
            ...
            logger.info(f"=== merge_oracle_wallets completed ===")
            return json.dumps(result_data, indent=2)
            
        except Exception as e:
            logger.error(f"Error merging wallets: {e}", exc_info=True)
            return json.dumps({
                "success": False,
                "error": str(e),
                "error_type": type(e).__name__
